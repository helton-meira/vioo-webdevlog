...
export default function Timeline({ posts }: { posts: PostData[] }) {
  ...
  // Extrair todas as tags únicas para criar os botões
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg transition-all text-sm font-medium border ${
              filter === null
                ...
                : "bg-slate-200 dark:bg-slate-800 border-transparent hover:border-primary/30"
            }`}
          >
            Todas
          </button>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all text-sm font-medium border ${
                filter === tag
                  ...
                  : "bg-slate-200 dark:bg-slate-800 border-transparent hover:border-primary/30 text-slate-600 dark:text-slate-300"
              }`}
            >
              {tagIcons[tag]}
              {tag}
      ...
      <section className="relative min-h-[400px]">
        ...
        <div className="space-y-16">
          {filteredPosts.length === 0 && (
            <p className="text-center text-slate-500 py-10 ml-20">Nenhuma atualização encontrada com esse filtro.</p>
          )}
          ...
          {filteredPosts.map((post) => {
            ...
            return (
              <div key={post.id} className="relative grid grid-cols-[80px_1fr] md:grid-cols-[140px_1fr] gap-8 group animate-in fade-in slide-in-from-bottom-4 duration-500">
                {/* Data */}
                <div className="text-right pt-2 pr-4 md:pr-10">
                  <p className="text-sm font-bold text-slate-500 dark:text-slate-400 capitalize">
                    {day} {month}
                  </p>
                ...
                <div className="relative pl-8 pb-4">
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl shadow-sm hover:shadow-md transition-all">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 leading-relaxed mb-4 text-sm whitespace-pre-line">
                      {post.content}
                    </p>
                    ...
                    <div className="flex items-center gap-4 text-xs font-medium text-slate-400">
                      {post.version && (
                        <span className="flex items-center gap-1">
                          <GitCommit size={14} /> {post.version}
                        </span>
                      )}
                      <span className="flex items-center gap-1">
                        <History size={14} /> Postado por @admin
                      </span>
                    </div>
                  </div>
                </div>
